function [I_ac_p_rms,I_sw_inner_rms_actual,I_sw_outer_rms_actual,...
            ...
            delta_t2,I_3,I_2,D_loss,I_Lf_ripple,ratio_div_sw_actual,Eoss_23,E_remain] = circuit_state(V_in,V_o,I_o,L_f,N_p,...
...
%
    Coss_vs_Vds = csvread('Coss_vs_Vds_C3M0160120D.csv');
    Cd_vs_Vr = csvread('Cd_vs_Vr_IDWD10G120C5.csv');
    Coss_nom = Coss_vs_Vds(end,2) / (1e12);
    Eoss_23 = Coss_nom * (V_in / 2) ^ 2;
    
    t_stp = 100e-12; % time step 0.1 ns
    t_total = 50e-9; % total time span 100 ns
    ...
    E_1 = zeros(data_length,1);
    E_4 = zeros(data_length,1);
    ...
    for i = 1:1:data_length
        ...
        if i == 1
            ...
            [val,idx]=min(abs(Coss_vs_Vds(:,1) - v_ds_1(i,1)));
            Coss1 = Coss_vs_Vds(idx,2);
            [val,idx]=min(abs(Coss_vs_Vds(:,1) - v_ds_4(i,1)));
            Coss4 = Coss_vs_Vds(idx,2);

            E_1(i,1) = 0.5 * Coss1 * v_ds_1(i,1) * v_ds_1(i,1) / (1e12);

            E_4(i,1) = 0.5 * Coss4 * v_ds_4(i,1) * v_ds_4(i,1) / (1e12);
        else
    ...
    N_charge  = floor(T_co / t_stp);
    E_remain = 0.5 * Coss_nom * (V_in / 2) ^ 2 - E_4(N_charge);
